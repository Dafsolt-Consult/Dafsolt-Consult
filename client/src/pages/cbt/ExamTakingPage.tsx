...
export function ExamTakingPage() {
  ...
  useEffect(() => {
    api
      .get<AttemptState>(`/cbt/attempts/${attemptId}`)
      .then(({ data }) => {
        setState(data);
  ...
  const submit = useCallback(async () => {
    ...
    try {
      await api.post(`/cbt/attempts/${attemptId}/submit`);
      setSubmitted(true);
    } catch (err) {
  ...
  async function saveAnswer(questionId: string, payload: { selectedOptionId?: string; textAnswer?: string }) {
    ...
    try {
      await api.post(`/cbt/attempts/${attemptId}/answers`, { questionId, ...payload });
    } catch (err) {
      setError(apiErrorMessage(err));
    }
  }
  ...
  if (submitted) {
    return (
      <Card className="mx-auto max-w-lg text-center">
        ...
        <p className="mt-2 text-sm text-slate-500">Your answers have been recorded. Objective questions are graded instantly.</p>
        <Button className="mt-6" onClick={() => navigate("/cbt/available")}>
          Back to my exams
        </Button>
      </Card>
    );
